...
/** Stores expandable details for See-more buttons, keyed by message ID */
const detailsStore = new Map<string, string>()
  ...
  tools: [
    {
      name: 'reply',
      description:
        'Reply on Discord. Pass chat_id from the inbound message. Optionally pass reply_to (message_id) for threading, files (absolute paths) to attach, buttons for interactive responses, and details for expandable content.',
      inputSchema: {
        type: 'object',
        properties: {
          chat_id: { type: 'string' },
          ...
          buttons: {
            ...
            items: {
              type: 'object',
              properties: {
                id: { type: 'string', description: 'Button identifier, returned in button_response notification' },
                label: { type: 'string', description: 'Button display text' },
                style: {
                  type: 'string',
                  enum: ['success', 'danger', 'primary', 'secondary'],
                  description: 'Button color: success=green, danger=red, primary=blue, secondary=grey. Default: primary',
                },
              },
              required: ['id', 'label'],
            },
            description: 'Interactive buttons. Max 5 per message. User clicks are sent back as MCP notifications.',
          },
          details: {
            type: 'string',
            description: 'Expandable detail text. When provided, a "See more" button is automatically added.',
          },
        },
  ...
  try {
    switch (req.params.name) {
      case 'reply': {
        ...
        // Build interactive button row if requested
        ...
        if (buttons && buttons.length > 0) {
          ...
          for (const btn of buttons.slice(0, 5)) {
            row.addComponents(
              new ButtonBuilder()
                .setCustomId(`opr:${btn.id}:pending`)
                .setLabel(btn.label)
                .setStyle(styleMap[btn.style ?? 'primary'] ?? ButtonStyle.Primary),
            )
          }
          components = [row]
        }
          ...
          for (let i = 0; i < chunks.length; i++) {
            ...
            const sent = await ch.send({
              ...
              ...(shouldReplyTo
                ...
                : {}),
              ...(isLastChunk && components.length > 0 ? { components } : {}),
            })
            noteSent(sent.id)
            sentIds.push(sent.id)

            // Fix up button customIds and store details now that we have the message ID
            if (isLastChunk && components.length > 0) {
              const msgId = sent.id
              if (details) {
                detailsStore.set(msgId, details)
              }
              const updatedRow = new ActionRowBuilder<ButtonBuilder>()
              for (const comp of row!.components) {
                const oldId = comp.data.custom_id!
                const newId = oldId.replace(':pending', `:${msgId}`)
                updatedRow.addComponents(
                  ButtonBuilder.from(comp.data).setCustomId(newId),
                )
              }
...
client.on('interactionCreate', async (interaction: Interaction) => {
  ...
  // --- opr: interactive buttons ---
  const oprMatch = /^opr:([\w-]+):(\d+)$/.exec(interaction.customId)
  if (oprMatch) {
    const access = loadAccess()
    ...
    const [, buttonId, messageId] = oprMatch

    if (buttonId === 'more') {
      const storedDetails = detailsStore.get(messageId)
      if (!storedDetails) {
      ...
      const updatedComponents = interaction.message.components.map(
        actionRow => ActionRowBuilder.from<ButtonBuilder>(actionRow).setComponents(
          actionRow.components
            .filter(c => c.customId !== `opr:more:${messageId}`)
      ...
      detailsStore.delete(messageId)
      return
    }

    const clickedButton = interaction.message.components
      .flatMap(actionRow => actionRow.components)
      .find(c => c.customId === interaction.customId)
    const label = clickedButton && 'label' in clickedButton ? clickedButton.label : buttonId
    ...
    void mcp.notification({
      ...
      params: {
        content: `Button clicked: ${label}`,
        meta: {
          source: 'button_response',
          button_id: buttonId,
  ...
  // --- perm: permission buttons (original logic, untouched) ---
